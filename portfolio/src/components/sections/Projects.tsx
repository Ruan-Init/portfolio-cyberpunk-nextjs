...
const projects = [
  {
    num: '001', icon: '🌐', badge: 'WEB APP', bg: 'linear-gradient(135deg,#0a0f1e,#0d1a2e,#0a1525)',
  ...
  {
    num: '002', icon: '🎮', badge: '3D / WebGL', bg: 'linear-gradient(135deg,#1a0a1e,#2a0d2e,#1a0525)',
    title: 'CyberWorld 3D',
  ...
  {
    num: '003', icon: '🤖', badge: 'AI / MOBILE', bg: 'linear-gradient(135deg,#0a1a0e,#0d2a16,#051a0a)',
...
function ProjectCard({ p, index, visible }: { p: typeof projects[0]; index: number; visible: boolean }) {
  const { t } = useLang()
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: 'hidden', transition: 'all 0.45s', position: 'relative',
        opacity: visible ? 1 : 0,
    ...
    >
      {/* Thumb */}
      <div style={{ height: 170, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: p.bg }}>
        {/* Grid */}
        <div style={{
        ...
        <div style={{ fontSize: '3.5rem', opacity: 0.65, position: 'relative', zIndex: 1 }}>{p.icon}</div>
        <div style={{
          position: 'absolute', top: 14, right: 14,
          fontFamily: 'Share Tech Mono, monospace', fontSize: '0.58rem',
          padding: '4px 10px', background: 'rgba(255,0,110,0.15)', border: '1px solid #ff006e',
          color: '#ff006e', letterSpacing: 2,
        }}>{p.badge}</div>
      </div>
      ...
      <div style={{ padding: 26 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 18 }}>
          {p.tags.map(tag => (
            <span key={tag} style={{
              fontFamily: 'Share Tech Mono, monospace', fontSize: '0.58rem',
              padding: '3px 9px', background: 'rgba(0,245,255,0.06)', border: '1px solid rgba(0,245,255,0.2)',
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          {['Live Demo', 'GitHub'].map(l => (
            <a key={l} href="#" data-hover style={{
              fontFamily: 'Share Tech Mono, monospace', fontSize: '0.62rem', color: '#00f5ff',
              textDecoration: 'none', letterSpacing: 2, display: 'flex', alignItems: 'center', gap: 6,
...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={ref} style={{ position: 'relative', zIndex: 10, padding: '120px 60px 80px' }}>
      <div style={{ marginBottom: 70 }}>
        ...
        <h2 style={{ fontFamily: 'Orbitron, monospace', fontSize: 'clamp(1.8rem, 4vw, 3rem)', fontWeight: 900, color: '#fff', lineHeight: 1 }}>
          <span style={{ color: '#00f5ff', textShadow: '0 0 20px #00f5ff' }}>{t('Projetos', 'Projects')}</span>{' '}
          {t('Selecionados', 'Selected')}
        </h2>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 30 }}>
        {projects.map((p, i) => <ProjectCard key={p.num} p={p} index={i} visible={inView} />)}
      </div>
    </section>
  )
}
